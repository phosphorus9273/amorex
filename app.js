...
const  { serverOneABC, serverTwoABC, serverThreeABC } = require('file.js');
(async function (util, global, settings, Canvas, color, gameDraw, socketStuff) {
  ...
  window.onload = async () => {
    ...
    moreControls.addEventListener("click", () => {
      if (moreControlsLength) {
        for (var b = 0; b < moreControlsLength.length; b++)
          moreControlsLength[b].classList.add("hidden");
        moreControlsLength = null;
        moreControls.classList.remove("x");
      } else {
        moreControlsLength = document.querySelectorAll(
          "#controlSettings tr.hidden"
        );
        for (b = 0; b < moreControlsLength.length; b++)
          moreControlsLength[b].classList.remove("hidden");
  ...
  function startGame() {
    ...
    settings.graphical.lowResolution =
  ...
  function drawPoly(
    ...
    try {
      ...
      resizeEvent();
      console.error(
        "Uh oh, 'CanvasRenderingContext2D' has gotton an error! Error: " + e
      );
    }
  }
  ...
  const drawEntity = (
    ...
    if (render.expandsWithDeath && settings.graphical.fancyAnimations)
      drawSize *= 1 + 0.5 * (1 - fade);
    ...
    if (
      ...
      context.canvas.width = context.canvas.height =
        ((drawSize * m.position.axis) / ratio) * 2 + initStrokeWidth;
      xx =
  ...
  function drawHealth(x, y, instance, ratio, alpha) {
    ...
    if (instance.id !== gui.playerid && instance.nameplate) {
      ...
      if (global.GUIStatus.renderPlayerScores)
        drawText(
    ...
    for (let instance of global.entities) {
      ...
      for (let i in global.chats[instance.id]) {
        let chat = global.chats[instance.id][i],
          ...
          alpha = Math.max(
            !global.mobile ? 0 : 1,
            Math.min(1000, chat.expires - now) / 1000
          );
  ...
  function drawSelfInfo(spacing, alcoveSize, max) {
    ...
    let vspacing = 4.65;
    let len = 1.7 * alcoveSize; // * global.screenWidth;
    let height = 23;
  ...
  function drawMinimapAndDebug(spacing, alcoveSize, GRAPHDATA) {
    ...
    for (let entity of minimap.get()) {
      ...
      switch (entity.type) {
        case 2:
          ...
          drawGuiRect(
            x + ((entity.x - trueSize) / global.gameWidth) * len - 0.4,
            y + ((entity.y - trueSize) / global.gameHeight) * height - 1,
            ((2 * trueSize) / global.gameWidth) * len + 0.2,
            ((2 * trueSize) / global.gameWidth) * len + 0.2
          );
          break;
        case 1:
    ...
    drawGuiCircle(
      x + (global.player.cx / global.gameWidth) * len - 0,
    ...
    if (global.showDebug) {
      ...
      drawText(
        ...
        y - 50 - 1 * 14,
        10,
        orangeColor ? color.orange : color.guiwhite,
        "right"
      );
      drawText(
  ...
  function drawLeaderboard(spacing, alcoveSize, max) {
    ...
    for (let i = 0; i < lb.data.length; i++) {
      ...
      drawEntity(
        ...
        0 /*-Math.PI / 8*/,
        true
      ); // we dont wat to rotate the tanks img.
      // Move down
      y += vspacing + height;
    }
  }
  ...
  let getTips = () => {
    ...
    if (global.finalKillers.length) {
      txt +=
        "If you have died unfairly, you can get your score back by contacting a moderator.";
    } else {
  ...
  const gameDrawOldDead = () => {
    ...
    let ratio = util.getScreenRatio();
    scaleScreenRatio(ratio, true);
    let shift = global.enableSlideAnimation
      ? animations.deathScreen.get()
      : animations.deathScreen.getNoLerp();
    ctx.translate(0, -shift * global.screenHeight);
    let x = global.screenWidth / 2,
      y = global.screenHeight / 2 - 50;
    let len = 140,
  ...
  const gameDrawBeforeStart = () => {
    ...
    let shift = global.enableSlideAnimation
      ? animations.connecting.get()
      : animations.connecting.getNoLerp();
    ctx.translate(0, -shift * global.screenHeight);
    drawText(
      "Connecting...",
      global.screenWidth / 2,
  ...
  const gameDrawDisconnected = () => {
    ...
    let shift = global.enableSlideAnimation
      ? animations.disconnected.get()
      : animations.disconnected.getNoLerp();
    ctx.translate(0, -shift * global.screenHeight);
    drawText(
      "Disconnected",
      global.screenWidth / 2,
  ...
  const gameDrawError = () => {
    ...
    let shift = global.enableSlideAnimation
      ? animations.error.get()
      : animations.error.getNoLerp();
    ctx.translate(0, -shift * global.screenHeight);
    drawText(
  ...
  function animloop() {
    ...
    try {
      ...
      if (global.disconnected) {
        // Draw disconnection screen if the client lost connection to the server.
