...
const util = {
  ...
  pullJSON: (fileName, Protocol) => {
    return new Promise((resolve, reject) => {
      const url = `${window.serverAddProtocol}//${window.serverAdd}/lib/json/${fileName}.json`;
    //  console.log("Loading JSON from " + url); dont even think about it....
      ...
      fetch(url/*, { mode: "no-cors" }*/)
        ...
        .then((json) => {
         // console.log("JSON load from " + url + " complete"); nuh uh!
          console.log("JSON load from [REDACTED] complete"); // we do NOT want leaks,.
          resolve(json);
        })
        .catch((error) => {
       //   console.log("JSON load from " + url + " failed"); nope-
          reject(error);
        });
    });
  },
