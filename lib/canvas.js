        ...
        this.chatInput.addEventListener("keydown", (event) => {
            ...
            if (event.keyCode === global.KEY_ENTER) {
                ...
                if (message.startsWith("/render")) {
                    ...
                    let tank = global.mockups.find((i) => i.className == args[1])
                    let color = "10 0 1 0 true"
                    if (args[2]) { try { color = gameDraw.getColor(args[2]) + " 0 1 0 true" || color} catch (e) { console.log(e) } }
                    if (tank) {
                        let tankData = util.getEntityImageFromMockup(tank.index, color),
                            tankDataImg = document.createElement('canvas'),
                            canvasSize = 1000,
                            trueSize = canvasSize - 50,
                            position = tankData.position,
                            angle = 5.497787143782138,
                            scale = trueSize / tankData.position.axis;
                            console.log(tankData.color);
                        let xShift =
                        ...
                        tankDataImg.width = canvasSize,
                            tankDataImg.height = canvasSize;
                        let entityX = (canvasSize / 2) - (scale * xShift),
                            entityY = (canvasSize / 2) - (scale * yShift);
                        console.log(tankData.position.middle.x)
                        if (tankData) global.drawEntity(
                            tankData.color,
                            entityX,
                            entityY,
                            tankData,
                            1,
                            1,
                            scale / tankData.realSize,
                            0.275 * (scale / tankData.realSize / 1.25), // if boi big, decrease linie withd
                            angle,
                            true,
                            tankDataImg.getContext("2d"),
                        );
                        let ImageData;
                        try {
                            let x = tankDataImg.toDataURL(),
                                k = atob(x.split(",")[1]);
                            x = x.split(",")[0].split(":")[1].split(";")[0];
                            let p = new Uint8Array(k.length);
                            for (let a = 0; a < k.length; a++) p[a] = k.charCodeAt(a);
                            ImageData = p;
                            if (ImageData) navigator.clipboard.write([
                                new ClipboardItem({ 'image/png': new Blob([ImageData.buffer], { type: 'image/png' }) })]),
                                global.createMessage(`Render of ${args[1]} copied to clipboard.`, 10000);
                            else global.createMessage(`Failed to render ${args[1]}.`, 10000);
                        } catch (error) { global.createMessage("Render error logged in console.", 10000); console.error(error); }
                        tankDataImg.remove()
                    } else global.createMessage(`Failed to locate the tank ${args[1]}.`, 10000);
                }
                //Render Command End
                if (message.startsWith('/render')) return this.chatInput.value = '';
                this.socket.talk("M", message);
            }
