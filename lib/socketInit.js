    ...
    __s: {
      ...
      getProgress: () =>
        levelscore
          ? Math.min(
              1,
              Math.max(0, (sscore.get() - deduction) / (levelscore - deduction))
            )
          : 0,
      getScore: () => sscore.get(),
      getLevel: () => level,
...
const Entry = class {
  ...
  update(to) {
    this.name = to.name;
    this.bar = to.bar;
    if (typeof to.bar === "string" && to.bar.includes(", "))
      this.bar = +to.bar.split(", ")[0];
    this.color = to.color;
    this.index = to.index;
...
function Status() {
  ...
  return {
    ...
    getState: () => statState,
    getFade: () => {
      return statState === "dying" || statState === "killed"
        ? 1 - Math.min(1, (getNow() - statTime) / 360)
        : 1;
    },
    getColor: () => {
      return "#eb5454";
    ...
    getBlend: () => {
      let o =
        statState === "normal" || statState === "dying"
          ? 0
          : statState === "invuln"
          ? 0.125 + Math.sin((getNow() - statTime) / 60) / 8
...
const process = (z = {}) => {
  ...
  if (type & 0x01) {
    ...
    if (isNew) {
      z.render = {
        ...
        lastx:
          z.x -
          global.metrics.rendergap * settings.roomSpeed * (1000 / 30) * z.vx,
        lasty:
          z.y -
          global.metrics.rendergap * settings.roomSpeed * (1000 / 30) * z.vy,
        lastvx: z.vx,
        lastvy: z.vy,
        lastf: z.facing,
        f: z.facing,
  ...
  for (let i = 0; i < gunnumb; i++) {
    ...
    z.guns.setConfig(i, {
      ...
      direction,
      offset,
    }); // Load gun config into container
    if (time > global.player.lastUpdate - global.metrics.rendergap)
      z.guns.fire(i, power); // Shoot it
  }
  // Update turrets
  ...
  data: () => {
    ...
    for (let e of global.entities) {
      ...
      if (
        ...
        util.isInView(
          e.render.x - global.player.renderx,
          e.render.y - global.player.rendery,
          e.size,
          true
        )
      ) {
  ...
  gui: () => {
    ...
    if (indices.upgrades) {
      ...
      for (let i = 0, len = get.next(); i < len; i++) {
        ...
        gui.upgrades[i][2] = util.getEntityImageFromMockup(
          gui.upgrades[i][2],
          gui.color
        );
      }
...
const socketInit = (port) => {
  ...
  let socket = new WebSocket(
    `${protocols[window.serverAddProtocol]}${window.serverAdd}`
  );
  // Set up our socket
  ...
  socket.cmd = {
    ...
    talk: () => {
      ...
      socket.talk(
        "C",
        Math.round(global.target.x / ratio),
        Math.round(global.target.y / ratio),
        global.reverseTank,
        global.movement,
        o
      );
  ...
  socket.onopen = function socketOpen() {
    socket.open = true;
    global.message =
      "That token is invalid, expired, or already in use on this server. Please try another one!";
  ...
  socket.onmessage = async function socketMessage(message) {
    ...
    switch (m.shift()) {
      ...
      case "svInfo": // For debugging.
        ...
        if (global.showDebug)
          console.log(
            `mspt: ${global.mspt} total entities on screen: ${
              global.entities.length
            } Player X: ${global.player.renderx.toFixed(
              1
            )} Player Y: ${global.player.rendery.toFixed(1)}`
          );
        break;
      ...
      case "F": // to pay respects
        ...
        global.finalKills = [
          util.Smoothbar(0, 3),
          util.Smoothbar(0, 4.5),
          util.Smoothbar(0, 2.5),
          util.Smoothbar(0, 10),
        ];
  ...
  socket.onclose = () => {
    ...
    if (global.canThrowSyncClockError)
      global.message = "Failed to sync with the server. Please try again.";
    if (global.canThrowClosedMessage)
      global.message =
        "The connection has closed. Refresh to continue playing!";
  };
  // Notify about errors
